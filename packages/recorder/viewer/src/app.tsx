...
const deriveViewerSteps = (plan: ExecutedTestPlan | undefined): ViewerStepEvent[] => {
  ...
  return plan.steps.map((step) => ({
    ...
    title: step.title,
    status: step.status,
    startedAtMs: step.startedAt._tag === "Some" ? Number(step.startedAt.value) : undefined,
    endedAtMs: step.endedAt._tag === "Some" ? Number(step.endedAt.value) : undefined,
  }));
};
...
const ReplayViewer = ({
  ...
  const setupReplayScaling = () => {
    ...
    iframeObserver.observe(iframe, { attributes: true, attributeFilter: ["width", "height"] });
...
export const App = () => {
  ...
  const isLive = !AsyncResult.isSuccess(output) || !output.value.done;

  return (
    <div className="mx-auto max-w-[1200px]">
      <StepsPanel executedPlan={executedPlan} />
      <ReplayViewer events={rrwebEvents} steps={viewerSteps} live={isLive} />
    </div>
  );
};
